  ...
  try {
    ...
    if (error) {
      ...
      if (!alredyExist) {
        ...
        const userRole = await Roles.findOne({ _id: userObj.role });
        if (userRole.title !== "Client") {
          res.status(500).send("You have to buy Subscription.");
        }
        Users.findOneAndUpdate({ _id: alredyExist._id }, userObj, { new: true })
          .then(async (user) => {
            var userPlanObj = {};
      ...
      if (getuser && project) {
        if (
          ...
          if (taskCount <= userPlan.plan.texts) {
            ...
            res.status(500).send({
              message:
                "You have reached your Task limit please upgrade your package",
            });
          }
        }
        ...
        await session.commitTransaction();
        session.endSession();
        res.status(500).send({ message: "You are not a valid user" });
      }
    }
  } catch (err) {
    // emails.errorEmail(req, err);
