...
const subscriptonCheck = async () => {
  try {
     const plans = await UserPlans.find({})
       .populate("plan")
       .populate("subPlan");
     const today = dayjs();
...
const taskDeadlineCheck = async () => {
  try {
    const allTasks = await ProjectTask.find({ isActive: "Y" }).populate(["texter","lector","seo","metaLector"])
    ...
    for (const task of allTasks) {
      ...
      if (task.status.toLowerCase() === "ready to work" || task.status.toLowerCase() === "in progress" || task.status.toLowerCase() === "in rivision") {
        if (hoursDifference >= 48) {
          await ProjectTask.findOneAndUpdate({ _id: task._id }, {
            texter: null
          },{new: true})
      ...
      // lector
      if (task.status.toLowerCase() === "ready for proofreading" || task.status.toLowerCase() === "proofreading in progress") {
         if (hoursDifference >= 48) {
      ...
      // seo optimizer
      if (task.status.toLowerCase() === "ready for seo optimization" || task.status.toLowerCase() === "seo optimization in progress") {
         if (hoursDifference >= 48) {
           await ProjectTask.findOneAndUpdate(
             { _id: task._id },
             {
      ...
      // meta lector
      if (task.status.toLowerCase() === "ready for 2nd proofreading" || task.status.toLowerCase() === "2nd proofreading in progress") {
         if (hoursDifference >= 48) {
           await ProjectTask.findOneAndUpdate(
             { _id: task._id },
             {
